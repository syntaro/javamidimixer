 ...
 */
public class MX36StatusList {
    public static final MX36StatusList _instance = new MX36StatusList();
    ...
    public class Folder {
        ...
        synchronized boolean removeAll(MX36Status status) {
            ...
            for (int i = _listStatus.size() - 1; i >= 0; -- i) {
                ...
                else if (status.compareSurfacePosition(comp) == 0) {
                    _listStatus.remove(i);
                    removedFlag = true;
                }
            }
        ...
        synchronized MX36Status findBySurfacePosition(int port, int type, int row, int column) {
            for (int i = _listStatus.size() - 1; i >= 0; -- i) {
                MX36Status comp = _listStatus.get(i);
                if (comp.compareSurfacePosition(port, type, row, column) == 0) {
                    return comp;
                }
            }
            return null;
        ...
        synchronized int insertPoint(MX36Status status) {
            ...
            for (int i = 0; i < _listStatus.size(); ++ i) {
                ...
                int x = seek.compareSurfacePosition(status);
                if (x < 0) {
                    found = i + 1;
                    continue;
                }
