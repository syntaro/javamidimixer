...
public class ParamsOfNavigator<T> {
    public ParamsOfNavigator(INavigator navi) {
        _navi = navi;
        _mode = MODE_VIEWER;
        _returnCode = 0;
        _value = null;
    ...
    public void closeWithApprove(INavigator panel, T value) {
        ...
        _value = value;
        if (_navi instanceof INavigator2) {
            System.out.println("validation " + value);
            INavigator2<T> validate = (INavigator2<T>)_navi;
            if (validate.validatePromptResult(_value) == false) {
                return;
            }
        }
