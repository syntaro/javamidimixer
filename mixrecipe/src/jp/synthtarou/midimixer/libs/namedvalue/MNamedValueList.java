 ...
 */
public class MNamedValueList<T> extends ArrayList<MNamedValue<T>> implements ListModel, ComboBoxModel {

    boolean _ignoreCase = false;

    public MNamedValueList() {
        super();
    }
    ...
    public boolean addNameAndValue(String name, T value) {
        boolean x = add(new MNamedValue(name, value));
        return x;
    }
    
    @Override
    public boolean add(MNamedValue<T> value) {
    ...
    public T readComboBox(JComboBox box) {
        ...
        MNamedValue<T> item = (MNamedValue)model.getSelectedItem();
        if (item != null) {
            return item._value;
        }
        return null;
    ...
    public void writeComboBox(JComboBox<?> box, T value) {
        ...
        for (int i = 0; i < model.getSize(); ++ i) {
            MNamedValue<T> item = (MNamedValue)model.getElementAt(i);
            if (item != null) {
                if (value == null) {
                    if (item._value == null) {
