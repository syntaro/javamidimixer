...
import jp.synthtarou.midimixer.mx35cceditor.ccxml.navigator.NavigatorForText;
import jp.synthtarou.midimixer.mx35cceditor.ccxml.navigator.NavigatorForNodeAttribute;
...
public class EditorForXMLTag extends javax.swing.JPanel {
    ...
    public void setTargetFile(CXFile file) {
        ...
        _file = file;
        MXWrapList<CXNode> model = new MXWrapList();

        new MXAttachComboRendererShrinkable(jComboBox1);
        for (CXNode module : file.listModules()) {
            String name = module._listAttributes.valueOfName("Name");
    ...
    void fillTreeSub(CXNode root) {
        _treeModel = new CXTreeModel(root);
    ...
    private void jTextAreaTextContentMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTextAreaTextContentMousePressed
        ...
        NavigatorForText prompt = new NavigatorForText(_selection.getTextContent(), "Text Of Tag");
        NavigatorUtil.showPrompt(this, prompt);
        String result = prompt.getParamsOfNavigator().getApprovedValue();
        if (result != null) {
    ...
    private void jButtonAddAttributeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAddAttributeActionPerformed
        ...
        NavigatorForNodeAttribute prompt = new NavigatorForNodeAttribute(_selection, null, null);
        NavigatorUtil.showPrompt(this, prompt);
        MXWrap<String> result = prompt.getParamsOfNavigator().getApprovedValue();
        if (result == null) {
            return;
        }
    ...
    private void jButtonEditAttributeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditAttributeActionPerformed
        ...
        NavigatorForNodeAttribute prompt = new NavigatorForNodeAttribute(_selection, name, value);
        NavigatorUtil.showPrompt(this, prompt);
        MXWrap<String> result = prompt.getParamsOfNavigator().getApprovedValue();
        if (result != null) {
            ...
            if (name.equals(result.name)) {
                ...
                _selection._listAttributes.set(prevIndex, new MXWrap<>(name, result.value));
            } else {
                int prevIndex = _selection._listAttributes.indexOfName(name);
