...
public abstract class TableModelWithNavi extends AbstractTableModel {
    ...
    public void startEdit(Container parent, int row) {
        INavigator editor = getRowEditor(parent, row);
        NavigatorUtil.showPrompt(parent, editor);
    }
    
    public boolean startCellEdit(Container parent, int row, int column) {
        INavigator editor = getCellEditor(parent, row, column);
        NavigatorUtil.showPrompt(parent, editor);
        Object result = editor.getParamsOfNavigator().getApprovedValue();
        if (result != null) {
    ...
    public abstract INavigator getRowEditor(Component parent, int rowIndex);

    public abstract INavigator getCellEditor(Component parent, int rowIndex, int cellIndex);
}
