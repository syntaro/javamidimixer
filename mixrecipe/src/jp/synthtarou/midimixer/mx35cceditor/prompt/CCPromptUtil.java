...
public class CCPromptUtil {
    ...
    public static void showPanelForTest(Container parent, JPanel panel) {
        ...
        modal.pack();
        modal.setModal(true);
        if (panel instanceof IPromptPanel) {
            IPromptPanel prompt = (IPromptPanel) panel;
            modal.setTitle(prompt.getPanelTitle());
            modal.setSize(prompt.getPanelSize());
        }
    ...
    public static void closeAnyway(IPromptPanel prompt) {
    ...
    static class CloseFook extends WindowAdapter {
        ...
        JDialog _owner;
        IInputPromptPanel<Object> _input;

        CloseFook(JDialog owner, IInputPromptPanel input) {
            _owner = owner;
            _input = input;
        }
    ...
    public static Object showPrompt(Container parent, IPromptPanel prompt) {
        Container cont = MXUtil.getOwnerWindow(parent);
        String title = prompt.getPanelTitle();

        IInputPromptPanel _asInput = null;

        if (prompt instanceof IInputPromptPanel) {
            _asInput = (IInputPromptPanel) prompt;
        }
        ...
        Dimension size = prompt.getPanelSize();
        if (size != null) {
            child.setSize(size);
        }
        MXUtil.centerWindow(child);
