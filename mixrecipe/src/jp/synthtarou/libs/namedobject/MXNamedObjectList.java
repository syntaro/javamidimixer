 ...
 */
public class MXNamedObjectList<T> extends ArrayList<MXNamedObject<T>> implements ListModel, ComboBoxModel {

    boolean _ignoreCase = false;

    public MXNamedObjectList() {
        super();
    }
    ...
    public boolean addNameAndValue(String name, T value) {
        boolean x = add(new MXNamedObject(name, value));
        return x;
    }
    
    @Override
    public boolean add(MXNamedObject<T> value) {
    ...
    public T readComboBox(JComboBox box) {
        ...
        MXNamedObject<T> item = (MXNamedObject)model.getSelectedItem();
        if (item != null) {
            return item._value;
        }
        return null;
    ...
    public void writeComboBox(JComboBox<?> box, T value) {
        ...
        for (int i = 0; i < model.getSize(); ++ i) {
            MXNamedObject<T> item = (MXNamedObject)model.getElementAt(i);
            if (item != null) {
                if (value == null) {
                    if (item._value == null) {
