...
public class PromptTableModelEx extends PromptTableModel {
    ...
    @Override
    public IPromptPanel getRowEditor(Component parent, int rowIndex) {
        IPromptPanel editor = new AnotherTableEditPanel(this, rowIndex);
        return editor;
    }

    @Override
    public IInputPromptPanel getCellEditor(Component parent, int rowIndex, int columnIndex) {
        String text = getValueAsText(rowIndex, columnIndex);
        IInputPromptPanel<String> editor = new CCTextPrompt(text, "Text");
        return editor;
    }
