#include "pch.h"
#include "MXLIB03FluidSynth.h"
#include <locale>
#include <codecvt>

JavaVM* _javaVM;
jclass _javaClass = NULL;
#pragma comment(lib, "C:/github/fluidsynth-2.4.6-win10-x64/lib/libfluidsynth-3.lib")
//#pragma comment(lib, "C:/github/fluidsynth-2.4.2-win10-x64/lib/libfluidsynth.a")
...
fluid_handle_t* _allHandle = NULL;
...
int createFluidSynth() {
	...
	for (int i = 0; i < 20; ++i) {
		if (_allHandle[i].settings == NULL) {
			_allHandle[i].settings = new_fluid_settings();
			_allHandle[i].synth = NULL;
...
jint JNICALL jfluid_allocate(JNIEnv* env, jobject obj)
{
	...
	if (handle != NULL) {
		//fluid_settings_setstr(handle->settings, "synth.audio-driver", "dsound");
...
void JNICALL jfluid_free(JNIEnv* env, jobject obj, jint id)
{
	fluid_handle_t* handle = getFluid(id);
	if (handle != NULL) {
		if (handle->synth != NULL) {
...
jboolean JNICALL jfluid_open(JNIEnv* env, jobject obj, jint id)
{
	fluid_handle_t* handle = getFluid(id);
...
extern void JNICALL jfluid_unload_font(JNIEnv* env, jobject obj, jint id);

jboolean JNICALL jfluid_load_font(JNIEnv* env, jobject obj, jint id, jstring path)
{
	fluid_handle_t* handle = getFluid(id);
	if (handle != NULL && handle->synth != NULL) {
		if (handle->soundfont_id > 0) {
			jfluid_unload_font(env, obj, id);
		}
		MBString file(env, path);
...
void JNICALL jfluid_fill_buffer(JNIEnv* env, jobject obj, jint id, jshortArray shortarray, jint len) {
	fluid_handle_t* handle = getFluid(id);
	if (handle != NULL && handle->synth != NULL) {
		...
		jshort* arr = env->GetShortArrayElements(shortarray, NULL);
		if (!arr) {
			return;
		}
		jint a = fluid_synth_write_s16(handle->synth, len / 2, arr, 0, 2, arr, 1, 2);

		env->ReleaseShortArrayElements(shortarray, arr, 0);
	}
}
...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    jclass c = env->FindClass("jp/synthtarou/midimixer/windows/MXLIB03FluidSynth");
    ...
    */
    ...
    static JNINativeMethod methods[] = {
		{ (char*)"allocate", (char*)"()I", reinterpret_cast<void*>(jfluid_allocate)},
		{ (char*)"free", (char*)"(I)V", reinterpret_cast<void*>(jfluid_free)},
		{ (char*)"open", (char*)"(I)V", reinterpret_cast<void*>(jfluid_open)},
		{ (char*)"close", (char*)"(I)V", reinterpret_cast<void*>(jfluid_close)},

		{ (char*)"loadFont", (char*)"(ILjava/lang/String;)Z", reinterpret_cast<void*>(jfluid_load_font)},
		{ (char*)"unloadFont", (char*)"(I)V", reinterpret_cast<void*>(jfluid_unload_font)},
		...
		{ (char*)"systemReset", (char*)"(I)V", reinterpret_cast<void*>(jfluid_system_reset)},
